...
download_file() {
  ...
  if [ "$DOWNLOADER" = "curl" ]; then
    ...
    if curl -fsSL --http1.1 -o "$download_output" "$download_url"; then
      return 0
    fi
...
install_fzf() {
  ...
  if download_file "$fzf_url" "$fzf_temp/fzf_archive"; then
    ...
    if [ -f "$fzf_temp/$fzf_binary" ]; then
      ...
      printf "${GREEN}✓ fzf installed successfully${NC}\n"
...
if [ "$OS" = "linux" ] && is_android; then
  ...
  case $OS in
    ...
    *)
      ...
      printf "${BLUE}https://github.com/Teckwin/forgecode#installation${NC}\n"
      exit 1
      ;;
  esac
fi
...
if [ "$VERSION" = "latest" ]; then
  DOWNLOAD_URLS="https://github.com/Teckwin/forgecode/releases/latest/download/forge-$TARGET$TARGET_EXT"
else
  DOWNLOAD_URLS="https://github.com/Teckwin/forgecode/releases/download/$VERSION/forge-$TARGET$TARGET_EXT"
  case "$VERSION" in
    v*) ;;

    *)
      DOWNLOAD_URLS="$DOWNLOAD_URLS https://github.com/Teckwin/forgecode/releases/download/v$VERSION/forge-$TARGET$TARGET_EXT"
      ;;
  esac
fi
